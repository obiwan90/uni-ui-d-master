<template>
	<view v-if="isShow">
		<page-container v-if="closePageContainer" :show="isShow" :overlay="false" @beforeleave="beforeleave"></page-container>
	</view>
	<view class="listingProduct-layout">
		<!-- header -->
		<custom-header :backgroundColor="'#F3F2F6'">
			<template #name>定价上架</template>
		...
		<scroll-view scroll-y="true" style="height: 1150rpx; width: 100%;">
				...
				<template #image>
					<view style="width: 750rpx;height: 50%;display: flex;align-items: center;justify-content: center;">
						<view style="display: flex;flex-direction: column;gap: 28rpx;align-items: center;">
							<image src="https://www.xzxskj.com/xcximg/icon_dingjia@3x.png" mode="widthFix" style="width: 144rpx; height: 144rpx;"></image>
			...
			<view v-if="receivedData" class="edit-price">
				<view class="title">
					<!--  <text v-if="hasAbnormalities" style="font-size: 22rpx;opacity: 0.7;color: #669CFD;">{{该商品质检报告存在异常，建议降价销售}}</text> -->
					<text>定价金额 <text v-if="hasAbnormalities" style="font-size: 22rpx;opacity: 0.7;color: #669CFD;">该商品质检报告存在异常，建议降价销售</text></text>
				</view>
				<view class="content">
					<view style="display: flex; align-items: center">
						<image src="https://www.xzxskj.com/xcximg/qian@3x.png" mode="aspectFill"></image>
						<input type="number" placeholder="请输入批发金额" v-model="ListingPrice" />
					</view>
					<view style="display: flex; align-items: center" @click="toggleSpecialPrice">
...
const closePageContainer = ref(true)
const data = ref([
	{
		title: '上传图片',
		tips:'* 机身背面为封面图',
...
const hasAbnormalities = computed(() => {
