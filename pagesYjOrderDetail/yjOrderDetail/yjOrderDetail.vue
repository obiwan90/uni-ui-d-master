<template>
	<view class="yjOrder-layout">
		...
		<view class="menu-xx" v-if="getBtns(orderInfo.zt).length">
			<view class="card-xx" v-for="(btn, btnIndex) in getBtns(orderInfo.zt)" :key="btnIndex" @click="handleBtnClick(btn)">
				...
				<view class="product-card-r">
					...
					<view class="mid">
						<view class="version-tap"> 
							<text v-if="orderInfo.goodsinfos.zt==='1'">{{formatGoodsInfoTabs(orderInfo)}}</text>
