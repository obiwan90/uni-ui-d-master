				...
				<view style="position: absolute;left: 20rpx;bottom: 20rpx;width: 100%;display: flex;align-items: center;">
					<van-uploader 
						...
						<view class="uploader-slot">
							<image class="uploader-icon" style="height: 162rpx;width: 162rpx;" src="https://www.xzxskj.com/xcximg/img_add_img@3x.png" mode="aspectFill"></image>
						</view>
					</van-uploader>
				</view>
...
const price = ref(props.totalPrice || 0);
const remark = ref('')
...
const fpbkSubmitAcComfirm =async (id)=>{
	...
	const data = {
		id:JSON.stringify(props.ddxx),//[{"orderid":"1","goodsid":"1,3"},{"orderid":"9","goodsid":"3"}]
	...
	await debouncedFpbkSubmit(data)
	
}
const debouncedFpbkSubmit = debounce(async(data) => {
	await fpbkSubmitAc(data)
},400,true)
const fpbkSubmitAc = async(data)=>{
	const res = await fpbkSubmit(data)
	if(res.code==='200'){
		uni.showToast({
			title:'操作成功',
