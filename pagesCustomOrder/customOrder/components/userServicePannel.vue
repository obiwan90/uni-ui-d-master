<template>
  <view class="userServicePannel-layout1">
	  <!-- 预约发货 -->
    ...
    <view v-if="thfs === '1'" style="margin-top: 20rpx; width: 710rpx; background: #FFFFFF; border-radius: 20rpx; padding: 28rpx 24rpx; display: flex; justify-content: space-between; align-items: center;">
      ...
      <view class="example-body" style="display: flex; align-items: center; gap: 10rpx; justify-content: center;">
        <picker mode="selector" :range="updatedTimeRange" @change="onTimeRangeChange">
...
//   const newTimeRange = timeRange.value.filter(range => {
//     const [start, end] = range.split('-');
//     console.log(`Checking range: ${start} - ${end} against currentTime: ${currentTime}`);
//     return currentTime > start && currentTime < end;
//   });
...
//   return timeRange.value.slice(startIndex);
// };
// const filteredTimeRange = timeRange.value.filter(range => {
// 	const currentTime = getCurrentTime();
//     const [start, end] = range.split('-');
...
const filteredTimeRange = timeRange.value.filter(range => {
    ...
    const [start, end] = range.split('-');
    console.log(`Comparing: start=${start}, end=${end}, currentTime=${currentTime}`);
    return isToday(date.value)?(currentTime >= start && currentTime <= end):range; // 当前时间在范围内
});
