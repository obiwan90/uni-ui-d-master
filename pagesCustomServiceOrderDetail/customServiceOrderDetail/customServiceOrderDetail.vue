<template>
	<view class="customServiceOrderDetail-layout" v-if="infos">
		<custom-header>
			<template #name></template>
			<template #fill></template>
		</custom-header>
		<scroll-view scroll-y="true" style="height: 1400rpx;">
		<uv-alert v-if="infos.cwshinfos&&infos.cwshinfos.zt=='1'" type="warning" description="等待审核" closable></uv-alert>
		...
		<view @click="handleBtnClick" v-if="infos.shlx==='2'&&infos.tyzt==='3'" style="margin: 30rpx 0;width: 174rpx;height: 60rpx;background: #FFFFFF;border-radius: 10rpx 10rpx 10rpx 10rpx;display: flex;justify-content: center;align-items: center;gap: 4rpx;">
		...
		<view @click="handleBtnClick" v-if="infos.shlx==='4'&&infos.tyzt==='3'" style="margin: 30rpx 0;width: 174rpx;height: 60rpx;background: #FFFFFF;border-radius: 10rpx 10rpx 10rpx 10rpx;display: flex;justify-content: center;align-items: center;gap: 4rpx;">
			<image style="height: 32rpx;width: 32rpx;" src="https://www.xzxskj.com/xcximg/icon_wuliu@3x.png" mode="aspectFill"></image>
			<text class="text" style="font-weight: 400;font-size: 28rpx;color: #1F1F1F;">查看物流</text>
		</view>
		...
		<view v-if="infos.shlx!=='4'" style="margin-top: 20rpx;width: 710rpx;height: 100rpx;background: #FFFFFF;border-radius: 20rpx 20rpx 20rpx 20rpx;padding: 24rpx;display: flex;justify-content: space-between;align-items: center;">
			...
			<view style="display: flex;justify-content: center;align-items: center;">
				<text class="text" style="font-weight: bold; font-size: 32rpx; color: #1F1F1F;">¥ {{infos.bcje}}</text>
		...
		<view style="margin-top: 20rpx;width: 710rpx;background: #FFFFFF;border-radius: 20rpx 20rpx 20rpx 20rpx;display: flex;flex-direction: column;padding: 24rpx;">
			...
			<text class="text" style="margin-top: 24rpx; font-size: 28rpx; color: #1F1F1F;">{{infos.shyy}}</text>
			<view v-if="infos.shtt!==''" v-for="(item1,index) in infos.shtt.split(',')" :key="index" style="margin-top: 24rpx;display: flex;align-items: center;gap: 20rpx;" >
				<image v-if="item1" style="height: 144rpx;width: 144rpx;border-radius: 18rpx;" :src="item1" mode="aspectFill"></image>
			</view>
		</view>
		<!-- 售后信息 -->
		...
		<view v-if="infos.zt!=='2'" style="width: 750rpx;height: 182rpx;background: #FFFFFF;box-shadow: 0rpx 0rpx 20rpx 2rpx rgba(0,0,0,0.1);border-radius: 0rpx 0rpx 0rpx 0rpx;display: flex;justify-content: center;align-items: center;position: fixed;bottom: 0;left: 0;right: 0;">
			<view v-if="infos.zt!=='3'" style="display: flex;justify-content: center;align-items: center;gap: 14rpx;">
				<view style="width: 232rpx;height: 96rpx;background: #F2F3F6;border-radius: 20rpx 20rpx 20rpx 20rpx;display: flex;justify-content: center;align-items: center;" @click="handleClickDebounce('left')">
					<text style="font-weight: 400;font-size: 32rpx;color: #1F1F1F;">{{getButtonLabels(infos.shlx,infos.tyzt)?.btnLeft||''}}</text>
				</view>
				<view @click="handleClickDebounce('right')" style="width: 444rpx;height: 96rpx;background: #F2F3F6;border-radius: 20rpx 20rpx 20rpx 20rpx;display: flex;justify-content: center;align-items: center;background: linear-gradient( 135deg, #ED784B 0%, #EB4636 100%);border-radius: 20rpx 20rpx 20rpx 20rpx;">
					<text style="font-size: 32rpx;color: #FFFFFF;font-weight: 400;">{{getButtonLabels(infos.shlx,infos.tyzt)?.btnRight||''}}</text>
				</view>
			</view>
			
			<view v-else style="display: flex;justify-content: center;align-items: center;">
				<view  @click="handleClickDebounce('center')" style="width: 650rpx;height: 96rpx;background: #F2F3F6;border-radius: 20rpx 20rpx 20rpx 20rpx;display: flex;justify-content: center;align-items: center;background: linear-gradient( 135deg, #ED784B 0%, #EB4636 100%);border-radius: 20rpx 20rpx 20rpx 20rpx;">
					<text style="font-weight: 400;font-size: 32rpx;color: #FFFFFF;">同意售后</text>
				</view>
			</view>
		</view>
	...
	<uni-popup background-color="rgba(0,0,0,0)" :is-mask-click="true" ref="saiXuanPannelPopup" :safe-area="false" type="bottom" @close="handleClose" @change="change">
		<userServicePannel :infos='infos'  v-if="showSaiXuanPannel"  @subExpress="successAc"  @close="handleClose"></userServicePannel>
	</uni-popup>
</template>
...
const btns = ref([
	...
	{shlx:'2',tyzt:'0',btnLeft:'驳回售后',btnRight:'同意售后',btnCenter:''}, //6
	{shlx:'2',tyzt:'4',btnLeft:'驳回售后',btnRight:'已收到货,同意退款',btnCenter:''},
	
	{shlx:'4',tyzt:'0',btnLeft:'驳回售后',btnRight:'同意售后',btnCenter:''}, //5
...
const apiCustomServiceOrderOperationAc = async (lx) => {
	const param = {
		id: orderItem.value.id,
...
const handleClickDebounce = debounce((flag) => {
	handleClick(flag)
}, 300,true);
const handleClick = async (flag) => {
	...
	switch (flag) {
		case 'left':
			buttonName = getButtonLabels(infos.value.shlx, infos.value.tyzt)?.btnLeft || '';
			break;
		case 'right':
			buttonName = getButtonLabels(infos.value.shlx, infos.value.tyzt)?.btnRight || '';
			break;
		case 'center':
			buttonName = "同意售后";
			break;
...
const successAc =async (data)=>{
	apiCustomServiceOrderOperationAc(lx.value).then(res=>{
		if(res.code === 200)subExpress(data.expressid,data.id)
	});
}
...
.customServiceOrderDetail-layout{
	min-height: 100vh;
	background: #f3f2f6;
	box-sizing: border-box;
	padding-left: 20rpx;
