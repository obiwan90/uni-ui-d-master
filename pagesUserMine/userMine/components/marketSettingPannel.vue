  ...
  <view class="box">
    <view class="title">
      ...
      <view class="right" @click="goBack">
        <image src="https://www.xzxskj.com/Public/upload/66c97f9990c9331.png" mode="aspectFill"></image>
        <text>返回首页</text>
      </view>
    </view>
...
const toggleMessageType = (type) => {
  const index = selectedMessageTypes.value.indexOf(type);
  if (index === -1) {
    selectedMessageTypes.value.push(type);
  } else {
    selectedMessageTypes.value.splice(index, 1);
  }
  messageSettings.value.messageType = selectedMessageTypes.value
};

const isSelectedMessageType = (type) => {
  return selectedMessageTypes.value.includes(type);
};

const isReadyToPublish = ref(false);

watch([() => messageSettings.value.messageType, () => messageSettings.value.phone], () => {
